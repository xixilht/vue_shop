  ...
  data() {
    return {
      ...
      cateProps: {
        ...
        expandTrigger: 'hover',
        /* checkStrictly boolean值 属性说明:是否严格的遵守父子节点不互相关联,
        设为 true 可选择子级选项,若是 false 则只能选最后一级
        PS:商品分类允许只存在一级二级,后面不一定有三级,最后一项有可能是一级或二级或三级,这里的需求是:只能选第三级 */
        checkStrictly: true,
        value: 'cat_id', // 级联选择器中,选项的id值,后面需提交
        label: 'cat_name', // 级联选择器中,显示选项的文本值
        children: 'children' // 级联选择器,父子节点的嵌套属性
      },
      // 级联选择器双向绑定到的数组,选中的分类的Id数组
      selectedCateKeys: [],
      // 被激活的标签页的 页签名称
      activeName: 'many',
      // 动态参数的数据
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      if (this.selectedCateKeys.length !== 3) {
        ...
        // 清空静态属性表格数据
        this.onlyTableData = []
        // this.$message.warning('您选择的不是第三级分类,目前只允许为第三级分类设置相关参数!')
        return
      }
      ...
      console.log('参数的列表数据,不同参数包含不同列表数据,不同列表数据包含不同tag:', res.data)

      res.data.forEach(item => {
        item.attr_vals = item.attr_vals ? item.attr_vals.split(' ') : []
        /* 控制文本框的显示与隐藏
        注意：item.inputVisible 代表每一个 item对象 控制输入框显示与隐藏的属性值 inputVisible 都不一样 */
        item.inputVisible = false
        /* 文本框中输入的值,和上文同理,
        item.inputValue 代表每一个 item对象 控制输入框内容的 inputValue 都不一样 */
        item.inputValue = ''
      })
    ...
    showInput(row) {
      ...
      this.$nextTick(_ => {
        // 获得了 input dom对象,直接focus,获得焦点
        this.$refs.saveTagInput.$refs.input.focus()
      })
    },
    // 文本框失去焦点,或 按下了 Enter 键都会触发
