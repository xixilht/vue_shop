...
export default {
  ...
  methods: {
    ...
    login () {
      ...
      // this.$refs.loginFormRef.validate(valid => console.log(valid)) true/false
      // 考虑到 vue.js 和 小程序代码 要构建压缩打包最好还是写分号结尾比较好
      this.$refs.loginFormRef.validate(async valid => {
        if (!valid) return
        // const result = await this.$http.post('login', this.loginForm);
        // 解构赋值 result 对象中的 data属性（返回6个属性，只需要这个属性）
        const { data: res } = await this.$http.post('login', this.loginForm)
        if (res.meta.status !== 200) return this.$message.error('登录失败') // $message.error 是失败提示
        this.$message.success('登录成功') // $message.error 是成功提示，可参考 element-ui 官网
...
.avatar_box {
  ...
  border-radius: 50%;
  padding: 10px;
  box-shadow: 0 0 10px #ddd; /* 给头像边框添加向外扩散10像素颜色为#ddd的阴影 */
  position: absolute;
  left: 50%;
  transform: translate(-50%, -50%);
  background-color: #fff;
